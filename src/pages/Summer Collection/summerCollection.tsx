...
// Start animations when the user has scrolled 60% down the page
  ...
  useEffect(() => {
    document.body.classList.add("summer-collection-page");

    return () => {
      document.body.classList.remove("summer-collection-page");
    };
  }, []);

  useEffect(() => {
    fetchSummerCollection().then(items => {
  ...
  return (
    <div className="summer-collection">
      <Header />
      <div className="hero">
        <h1 style={{ opacity: headerOpacity }}>Summer Collection</h1>
      </div>
      <div className={`promotional summer`} ref={promotionalRef} style={{ opacity: promoOpacity }}>
